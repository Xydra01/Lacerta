...
- [ ] Feature flag / env e.g. `LACERTA_LLM_DECOMPOSE=0` (default **off**)
- [ ] `llm_manager.propose_job(state) -> JobSpec` only (typed; no narrative plan as SoT)
- [ ] Still `validate(job)` + surface allowlist before spawn
- [ ] Templates remain first: `if python_router.can_handle(state): … else: llm…`
- [ ] Cap manager steps; refuse unbounded replan loops
- [ ] Structured errors + optional respawn — no heal-tower prompt OS
- [ ] Harness: with flag **off**, all existing scenarios unchanged
- [ ] Harness or unit: with flag **on**, one non-template goal still produces valid JobSpecs and finishes or fails cleanly
- [ ] Document that free decompose is rare and for mid-size local models must stay narrow
...
- [ ] Templates before free planning (default)
- [ ] Manager still cannot call FS/web/shell
- [ ] No second orchestrator path for “LLM mode”
- [ ] No coach / heal towers
- [ ] Workers still ephemeral
- [ ] No MCP/Remote required for this phase
