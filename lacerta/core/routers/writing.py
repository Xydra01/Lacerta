...
class WritingFromSourcesTemplate:
    ...
    def next_job(self, state: MacroState) -> JobSpec | None:
        ...
        root = state.inputs.get("root")
        if not root:
            raise ValueError("tpl.writing.from_sources requires root")
        task_id = str(state.inputs.get("task_id") or f"writing-{uuid.uuid4().hex[:8]}")
        target = str(state.inputs.get("target_document") or "from_sources.md")
        job = JobSpec(
            job_id=f"write-sources-{uuid.uuid4().hex[:8]}",
            ...
            inputs={
                ...
                "brief": {
                    ...
                    "advance_when": "single_draft",
                    "target_document": target,
                    "title": str(state.inputs.get("title") or "Lacerta From Sources"),
                    "target_paragraphs": 2,
                },
            },
            tools=[],
        ...
        state.plan.append("write_from_sources")
        return job


WRITING_TEMPLATES: dict[str, Any] = {
    WritingShortTemplate.template_id: WritingShortTemplate(),
    WritingFromSourcesTemplate.template_id: WritingFromSourcesTemplate(),
}
