...
- [ ] Implement FS tools in `lacerta/workers/code_tools.py` (port kit §3)
  - [ ] `read_file`, `write_file`, `search_replace`, `list_dir`, `grep`, `find_files`
  - [ ] Optional stub/`run_command` allowlist (full use in L4)
  - [ ] Project-root guard + write size cap (`LACERTA_MAX_WRITE_CHARS`)
  - [ ] `search_replace` requires exactly one match
  - [ ] Grep output contract: `rel/path.py:12: content…` (port kit §4)
- [ ] `CODE_TOOL_REGISTRY` + `get_active_tools` (≤4 tools per JobSpec)
- [ ] Ollama schema prep: inline `$defs`, `additionalProperties: false`, optional strip descriptions (port kit §6)
- [ ] `build_worker_json_schema` — flat tool-name enums, max 1 tool/turn
- [ ] `run_worker_session` in `worker_runtime.py` (port kit §2)
  - [ ] Fresh messages every job; purge caller-side after `JobResult`
  - [ ] `max_turns` in Python (`LACERTA_WORKER_MAX_TURNS`, clamp 1–16)
  - [ ] One tool per turn; truncate tool outputs
  - [ ] Repeat-path OS block for `write_file` / `search_replace`
- [ ] Worker prompt rules: JSON + reasoning + one tool; `final_report` when done
- [ ] Session metrics → `logs/worker-sessions.jsonl` (port kit §7)
- [ ] Scenario `smoke_write_file` in harness (port kit §8.1)
- [ ] Disk `check_acceptance` for `check_file_glob` + `file_contains` (port kit §8.2)
- [ ] Gate runs smoke with `--runs 3`; require 3/3 pass
- [ ] Model tier stub optional (`check_worker_model`) — warn only if needed
...
- [ ] Code uses tool loop — **not** capability FSM / “dev recipes”
- [ ] Manager still has no FS tools (manager may remain stub)
- [ ] Worker context purged after job
- [ ] `max_turns` enforced in Python
- [ ] Disk acceptance is source of truth for smoke
